...
class Node {
  ...
  vy: number;
  pulsePhase: number;
  pulseFrequency: number = 0.015;

  constructor(x: number, y: number, seedIndex: number) {
    this.x = x;
    this.y = y;
    
    // Use deterministic values based on index for consistent behavior
    this.vx = Math.cos(seedIndex * 0.1) * 0.15;
    this.vy = Math.sin(seedIndex * 0.1) * 0.15;
  ...
  update(celebrationMode: boolean = false) {
    ...
    const padding = 5; // Prevent getting stuck at exactly the edge
    if (this.x < padding) {
      this.x = padding;
      this.vx = Math.abs(this.vx) * 0.8; // Reduced velocity after bounce
    ...
    if (this.y < padding) {
      this.y = padding;
      this.vy = Math.abs(this.vy) * 0.8; // Reduced velocity after bounce
      ...
      for (let i = 0; i < animState.nodes.length; i++) {
        for (let j = i + 1; j < animState.nodes.length; j++) {
          ...
          if (distance < connectionDistance) {
            ...
            const opacity = (1 - distance / connectionDistance) * 0.6 * pulseAverage;
  ...
  return (
    <canvas
      ...
      style={{
        ...
        width: '100%',
        height: '100%',
        zIndex: 1, // Lower z-index so content is above
        opacity: 0.15, // Subtle background to match homepage exactly
        mixBlendMode: 'screen'
      }}
    />
  );
