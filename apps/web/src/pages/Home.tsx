...
export default function Home() {
  ...
  const checkWalletStatus = async () => {
    ...
    try {
      const res = await fetch(
        `/api/rep/lookup-wallet?address=${encodeURIComponent(address.toLowerCase())}`,
        {
          credentials: "include",
        },
      );
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (sessionRes.ok) {
        ...
        if (
          sessionData.address &&
          sessionData.address.toLowerCase() === normalizedAddress
        ) {
          console.log("[LOGIN] Session valid! Looking up .rep name...");

          // Get .rep name info
          const checkRes = await fetch(
            `/api/rep/lookup-wallet?address=${encodeURIComponent(normalizedAddress)}`,
            {
              credentials: "include",
            },
          );
          ...
          if (
            checkData.ok &&
            checkData.walletFound &&
            checkData.reservationId
          ) {
            console.log(
              "[LOGIN] Redirecting to dashboard with existing session",
            );
          ...
          console.log(
            "[LOGIN] Session address mismatch, need to re-authenticate",
          );
        }
      } else {
  ...
  const handleConnectWallet = async () => {
    ...
    try {
      const connector =
        connectors.find((c) => c.name === "Coinbase Wallet") || connectors[0];
              ...
              <div className="content-column">
                ...
                {isConnected && address && (
                  <div className="wallet-status-home">
                    ...
                    <span className="wallet-address-home">
                ...
                <div className="hero-ctas">
                  <button
                    ...
                    style={{
                      opacity: isCheckingWallet || hasRepName ? 0.5 : 1,
                      cursor:
                        isCheckingWallet || hasRepName
                          ? "not-allowed"
                          : "pointer",
                    }}
                  >
