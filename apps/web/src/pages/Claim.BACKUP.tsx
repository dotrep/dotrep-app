...
export default function Claim() {
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/rep/check?name=${encodeURIComponent(canonicalName)}`,
          {
            credentials: "include",
          },
        );
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      console.log(
        "[CLAIM] Signature verified successfully, method:",
        verifyData.method,
      );
      ...
      if (!reserveRes.ok || !reserveData.reservationId) {
        if (reserveData.error === "wallet_already_has_rep") {
          throw new Error(
            `This wallet already owns ${reserveData.existingName}.rep (only one .rep per wallet)`,
          );
        }
  ...
  const getStatusMessage = () => {
    ...
    if (isAvailable === true)
      return (
        <span className="status-available">✓ {name}.rep is available</span>
      );
    return null;
  };
  ...
  const canClaim =
    isConnected &&
    name &&
    isValidName(canonicalizeName(name)) &&
    isAvailable === true &&
    !isClaiming;
